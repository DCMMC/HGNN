...
from datasets.cora import load_citation_data as source_select
...
def train_test_model(cfg):
    ...
    fts, lbls, idx_train, idx_val, idx_test, n_category, _, edge_dict = source_select(cfg)
    ...
    schedular = optim.lr_scheduler.MultiStepLR(optimizer,
                                               ...
                                               gamma=cfg['gamma'])
    # (DCMMC) 这两个 criterion 的效果差别太大了吧，NLLLoss 直接不收敛
    ...
    if idx_test is not None:
        ...
        test_acc_lvl, epo_lvl = test(model, model_wts_lowest_val_loss, fts, lbls, n_category, idx_test, edge_dict, G,
                                     device, cfg['test_time'])
        print('**** Model of best val acc ****')
        test_acc_bva, epo_bva = test(model, model_wts_best_val_acc, fts, lbls, n_category, idx_test, edge_dict, G,
                                     device, cfg['test_time'])
